...
class Transfer {
  ...
  _complexNameToDisplayName(p) {
    ...
    return `${p.firstName ? p.firstName : ""}${p.middleName ? " " + p.middleName : ""} ${p.lastName ? p.lastName : ""
      }`.trim();
  }
  ...
  _convertToTransferParty(party) {
    return {
      ...
      displayName:
        party.displayName ||
        `${party.firstName ? party.firstName : ""}${party.middleName ? " " + party.middleName : ""} ${party.lastName ? party.lastName : ""
          }`.trim(),
